      ...
      <div className="container-shell py-7">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">Professional storytelling infrastructure for travel creators.</p>
          </div>
          <nav className="flex items-center gap-5 text-sm">
            {footerLinks.map((item) => (
              <a key={item.label} href={item.href} className="inline-flex items-center gap-1 text-slate-400 transition hover:text-slate-100">
                {item.label}
